...
$$\begin{equation}\displaystyle\min_{f}\displaystyle\max_{e \in \mathcal{E}} \mathbb{E}_{\cancel{G \sim p(\mathbf{G}|\mathbf{e}=e)}}\bigg[\frac{1}{|V|}\sum_{v \in V}{\mathbb{E}_{y\sim p(\mathbf{y}|\mathbf{G_v}=G_v, \mathbf{e}=e)}[l(f(G_v),y)]}\bigg]\end{equation}$$
...
즉, 본 논문은 node-level task에서의 OOD 문제를 정의하고, 기존의 접근법과 달리 varience를 tackle하는 접근법을 통해 invariant 가정에 근거한 새로운 학습 방법 (_Explore-to-Extrapolate Risk Minimization; EERM_)을 제시합니다.
